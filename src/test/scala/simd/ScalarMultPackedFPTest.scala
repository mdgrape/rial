...
class ScalarMultPackedFPTest extends AnyFlatSpec
  ...
  def multTest(len : Int, xSpec : RealSpec, ySpec : RealSpec, zSpec : RealSpec, roundSpec : RoundSpec,
    n : Int, stage : PipelineStageConfig ) = {
      test(new ScalarMultPackedFPGeneric( len, xSpec, ySpec, zSpec, roundSpec, stage )) { c =>
        {
          var q  = new Queue[(BigInt,Seq[BigInt],Seq[BigInt])]
          ...
          for (gen <- List( ("Test Within (-128,128)",generateRealWithin(128.0,_,_)),
            ...
            for(i <- 1 to n+nstage) {
              ...
              val zrs  = Seq.tabulate(len)(i => {xr.multiply(zSpec, roundSpec, yrs(i))})
