...
class NegPackedFPTest extends AnyFlatSpec
  ...
  def negTest(len : Int, xSpec : RealSpec, zSpec : RealSpec, roundSpec : RoundSpec,
    n : Int, stage : PipelineStageConfig ) = {
      test(new NegPackedFPGeneric( len, xSpec, zSpec, roundSpec, stage, true )) { c =>
        {
          var q  = new Queue[(Seq[BigInt],Seq[BigInt])]
            ...
            for(i <- 1 to n+nstage) {
              ...
              if (i > nstage) {
                ...
                for (i <- 0 until len) {
                  ...
                  val zi = zis(i)
                  assert(zi == z0d, f"x=$xid%16x $zi%16x!=$z0d%16x")
                }
              }
            }
            q.clear()
