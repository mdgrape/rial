...
class ExpOnlyTest extends AnyFlatSpec
    with ChiselScalatestTester with Matchers with BeforeAndAfterAllConfigMap {
  ...
  private def runtest (
    ...
    val pipeconfig = stage.getString
    it should f"exp(x) pipereg $pipeconfig spec ${spec.toStringShort} $generatorStr " in {
      test( new ExpGeneric(false, spec, nOrder, adrW, extraBits, stage, false, false)).
        withAnnotations(Seq(VerilatorBackendAnnotation)) { c =>
        {
          ...
          // behavior in most cases. However, unfortunately, the definition of
          // "an idle cycle" is like: a cycle where input or output does not
          // change. Since we are testing exp function, if we take a negative
...
//             println("---------------------------------------------------------")
  ...
  runtest(float48Spec, nOrderFP48, adrWFP48, extraBitsFP48, MathFuncPipelineConfig.none, n, r,
    "Test Safe Positive [0, 127]", generateRealWithin(0.0, 127.0,_,_))
  runtest(float48Spec, nOrderFP48, adrWFP48, extraBitsFP48, MathFuncPipelineConfig.none, n, r,
    "Test Safe Negative [-127, -0]", generateRealWithin(-126.0, -0.0,_,_))

  runtest(float48Spec, nOrderFP48, adrWFP48, extraBitsFP48, simplePipeline, n, r,
    "Test Safe Positive [0, 127]", generateRealWithin(0.0, 127.0,_,_))
  runtest(float48Spec, nOrderFP48, adrWFP48, extraBitsFP48, simplePipeline, n, r,
    "Test Safe Negative [-126, -0]", generateRealWithin(-126.0, -0.0,_,_))
