...
class MathFuncATan2Stage2Test extends FlatSpec
      ...
      test( new MathFunctions(spec, nOrder, adrW, extraBits, stage, false, false)) { c =>
        {
          ...
          val refstage1  = ATan2Stage1Sim.atan2Stage1SimGeneric(
            ReciprocalSim.reciprocalTableGeneration( nOrder, adrW, spec.manW, spec.manW+extraBits, Some(maxCalcW), Some(maxCbit) ),
            _, _)
          val refstage2  = ATan2Stage2Sim.atan2Stage2SimGeneric(
            ATan2Sim.atanTableGeneration( nOrder, adrW, spec.manW, spec.manW+extraBits, Some(maxCalcW), Some(maxCbit) ),
            _, _, _, _)
