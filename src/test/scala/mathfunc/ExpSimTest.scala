...
class MathFuncExpSimTest extends AnyFunSuite with BeforeAndAfterAllConfigMap {
  var n = 1000000
...
//       val libc = Native.loadLibrary("c", classOf[libc]).asInstanceOf[libc]
      ...
      for(i <- 1 to n) {
        ...
        val zi   = MathFuncExpSim.expSimGeneric(/*isPow2*/ false, t, x )
        val zd   = zi.toDouble
        val erri = errorLSB(zi, z0r)
  ...
  expTest(pow2F32TableI, RealSpec.Float32Spec, n, r,
