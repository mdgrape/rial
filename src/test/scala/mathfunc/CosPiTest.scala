...
//
// Testing CosPi ucosg ChiselTest
//

class MathFuncCosPiTest extends FlatSpec
      ...
      test( new MathFunctions(spec, nOrder, adrW, extraBits, stage, false, false)) { c =>
        {
          ...
          val reference  = CosPiSim.cosPiSimGeneric( CosPiSim.cosPiTableGeneration( nOrder, adrW, spec.manW, spec.manW+extraBits, Some(maxCalcW), Some(maxCbit) ), _ )
          ...
          for(i <- 1 to n+nstage) {
            ...
            c.io.sel.poke(SelectFunc.selectCosPi)
