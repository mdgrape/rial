...
class SinOnlyTest extends AnyFlatSpec
  ...
  private def runtest (spec : RealSpec,
    ...
    val pipeconfig = stage.getString
    it should f"sin(x) pipereg $pipeconfig spec ${spec.toStringShort} $generatorStr " in {
      test( new SinCosGeneric(true, spec, nOrder, adrW, extraBits, stage, false, false)).
        withAnnotations(Seq(VerilatorBackendAnnotation)) { c =>
        {
...
//             println("------------------")
  ...
  // BF16
  // TODO: Since taylor expansion uses higher order terms, table is not needed
  //       in sincos if we have narrow mantissa bits. It is needed to add a way
  //       to set the order of taylor expansion or smarter way of calculating
  //       taylorThreshold.
//
//   val nOrderBF16 = 0
//   val adrWBF16 = 7
