...
//             println(f"current zi = (${zisgn }|${ziexp}(${ziexp - spec.exBias})|${ziman.toLong.toBinaryString})(${new RealGeneric(spec, zisgn.toInt, ziexp.toInt, ziman).toDouble})")
            ...
            if (i > nstage) {
              ...
              println(f"zi  = ${zisgn  }|${ziexp  }(${ziexp   - spec.exBias})|${ziman  .toLong.toBinaryString})(${new RealGeneric(spec, zisgn.toInt,   ziexp.toInt,   ziman).toDouble}), " +
                      f"ref = ${zrefsgn}|${zrefexp}(${zrefexp - spec.exBias})|${zrefman.toLong.toBinaryString})(${new RealGeneric(spec, zrefsgn.toInt, zrefexp.toInt, zrefman).toDouble})"
                )

              if(zrefexp == 0) {
                assert(ziexp == 0, f"x = ${xref}, "+
