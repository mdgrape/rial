...
//
// Testing Sigmoid using ChiselTest
//

class SigmoidTest extends AnyFlatSpec
    with ChiselScalatestTester with Matchers with BeforeAndAfterAllConfigMap {

  behavior of "Test sigmoid"
  ...
  private def runtest ( spec : RealSpec,
    ...
    it should f"sigmoid(x) pipereg $pipeconfig spec ${spec.toStringShort} $generatorStr with ${fncfg.getString}" in {
      test( new MathFunctions(fncfg, spec, nOrder, adrW, extraBits, stage, None, false, false)).
        withAnnotations(Seq(VerilatorBackendAnnotation)) { c =>
        {
          ...
          val sigmoidTable = SigmoidSim.tableGeneration( nOrder, adrW, spec.manW, spec.manW+extraBits, Some(maxCalcW), Some(maxCbit) )
          val reference    = SigmoidSim.sigmoidSimGeneric( sigmoidTable, _ )
...
//             println(f"====================================================")
            ...
            c.io.sel.poke(fncfg.signal(Sigmoid))
...
//             println(f"zi = ${zi}")
            ...
            if (i > nstage) {
              ...
              assert(zi == z0d, f"x = (${xidsgn}|${xidexp}(${xidexp - spec.exBias})|${xidman.toLong.toBinaryString})(${xidr.toDouble}), "+
                                ...
                                f"ref(${z0dsgn}|${z0dexp}(${z0dexp - spec.exBias})|${z0dman.toLong.toBinaryString})(${sigmoid(xidr.toDouble)})")
            }
          }
        }
      }
