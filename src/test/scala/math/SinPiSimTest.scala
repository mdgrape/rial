...
//           val zrefd = zref.toDouble
          ...
          println(f"x   = ${x0}, sin(x) = ${math.sin(Pi * x0)}")
          println(f"ref = ${z0}")
          println(f"sim = ${zd}")
  ...
  sinPiTest(sinPiF32TableI, RealSpec.Float32Spec, n, r,
    "Test Within [0, 2^-9]", generateRealWithin(0.0, pow(2.0, -9)-pow(2.0, -23),_,_))
  sinPiTest(sinPiF32TableI, RealSpec.Float32Spec, n, r,
    "Test Within [0, 2^-5]", generateRealWithin(0.0, pow(2.0, -5)-pow(2.0, -23),_,_))
  sinPiTest(sinPiF32TableI, RealSpec.Float32Spec, n, r,
    "Test Within [0, 2^-1]", generateRealWithin(pow(2.0, -5), 0.5,_,_))
...
//   val sinPiBF16TableI = SinPiSim.sinPiTableGeneration(0, 7, 7, 7 ) // [1,2) + [2,4) + 1.0
//
