...
class ReciprocalTest extends AnyFlatSpec
  ...
  private def runtest ( spec : RealSpec,
    ...
    it should f"reciprocal(x) pipereg $pipeconfig spec ${spec.toStringShort} $generatorStr ${fncfg.getString} " in {
      test( new MathFunctions(fncfg, spec, nOrder, adrW, extraBits, stage, None, false, false)).
        withAnnotations(Seq(VerilatorBackendAnnotation)) { c =>
        {
          ...
          for(i <- 1 to n+nstage) {
            ...
            c.io.sel.poke(fncfg.signal(Reciprocal))
  ...
  val recOnly = new MathFuncConfig(Seq(Reciprocal))

  runtest(RealSpec.Float32Spec, nOrderFP32, adrWFP32, extraBitsFP32, complexPipeline,
    n, r, "Test Within (-128,128)",generateRealWithin(128.0,_,_), recOnly)
  runtest(RealSpec.Float32Spec, nOrderFP32, adrWFP32, extraBitsFP32, complexPipeline,
    n, r, "Test All range",generateRealFull(_,_), recOnly )
  runtest(RealSpec.Float32Spec, nOrderFP32, adrWFP32, extraBitsFP32, complexPipeline,
    n, r, "Test Special values",generateSpecialValues(_,_), recOnly )

  val recAndATan2 = new MathFuncConfig(Seq(Reciprocal, ATan2Phase1, ATan2Phase2))

  runtest(RealSpec.Float32Spec, nOrderFP32, adrWFP32, extraBitsFP32, complexPipeline,
    n, r, "Test Within (-128,128)",generateRealWithin(128.0,_,_), recAndATan2)
  runtest(RealSpec.Float32Spec, nOrderFP32, adrWFP32, extraBitsFP32, complexPipeline,
    n, r, "Test All range",generateRealFull(_,_), recAndATan2 )
  runtest(RealSpec.Float32Spec, nOrderFP32, adrWFP32, extraBitsFP32, complexPipeline,
    n, r, "Test Special values",generateSpecialValues(_,_), recAndATan2 )
