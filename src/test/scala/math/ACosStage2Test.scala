...
class ACosStage2Test extends AnyFlatSpec
    ...
    it should f"acos(x) pipereg $pipeconfig spec ${spec.toStringShort} $generatorStr " in {
      test( new MathFunctions(fncfg, spec, nOrder, adrW, extraBits, stage, None, false, false)).
        withAnnotations(Seq(VerilatorBackendAnnotation)) { c =>
        {
          ...
          val acosF32TableI = ACosStage2Sim.acosTableGeneration(nOrder, adrW,
            spec.manW, spec.manW+extraBits, Some(maxCalcW), Some(maxCbit))

          val refacos1  = ACosStage1Sim.acosStage1SimGeneric(sqrtF32TableI, _ )
          val refacos2  = ACosStage2Sim.acosStage2SimGeneric(acosF32TableI, _, _, _ )
