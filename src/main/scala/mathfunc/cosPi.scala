...
class CosPiOtherPath(
  ...
  val out_of_bounds = ((exBias+1).U === io.x.ex && io.x.man =/= 0.U) || ((exBias+1).U < io.x.ex)
  val znan  = io.x.nan || io.x.inf || out_of_bounds

  val zzero = (io.x.ex === (exBias-1).U && io.x.man === 0.U)               || // x == 0.5
              (io.x.ex === (exBias  ).U && io.x.man === (1 << (manW-1)).U) || // x == 1.5
              (    yex === 0.U          &&     yman === 0.U)                  // y == 0
  val zone  =  io.x.zero                             || // x == 0
              (io.x.ex === (exBias  ).U && io.x.man === 0.U) || // x == 1
              (io.x.ex === (exBias+1).U && io.x.man === 0.U) || // x == 2
              (    yex === (exBias-1).U &&     yman === 0.U)    // y == 0.5

  val zSgn = MuxCase(0.U(1.W), Seq(
    (io.x.ex === (exBias  ).U  && !zzero)                 -> ~(io.x.man(manW-1)),
    (io.x.ex === (exBias-1).U  && !zzero)                 -> 1.U(1.W),
    ((exBias-manW).U < io.x.ex && io.x.ex < (exBias-1).U) -> 0.U(1.W)
  ))
