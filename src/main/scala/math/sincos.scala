...
class SinCosGeneric(
  ...
  assert(sincosPre.io.dx.getOrElse(0.U) === 0.U || enPCReg)
  assert(sincosTab.io.cs.asUInt === 0.U         || enPCGapReg)
  ...
  if(order != 0) {
    polynomialEval.io.dx.get := ShiftRegister(sincosPre.io.dx.get, pcGap)
  }
